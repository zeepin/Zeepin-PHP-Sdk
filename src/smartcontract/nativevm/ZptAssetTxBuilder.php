...
namespace zeepin\smartcontract\nativevm;
...
use zeepin\core\transaction\Transfer;
...
class ZptAssetTxBuilder
{
  const ZPT_CONTRACT = '0000000000000000000000000000000000000001';
  const GALA_CONTRACT = '0000000000000000000000000000000000000002';

  public function getTokenContract(string $tokenType)
  {
    if ($tokenType === Constant::$TOKEN_TYPE->get('ZPT')) {
      return new Address(self::ZPT_CONTRACT);
    } else if ($tokenType === Constant::$TOKEN_TYPE->get('GALA')) {
      return new Address(self::GALA_CONTRACT);
    }
  ...
  public function makeWithdrawGalaTx(
    Address $from,
    Address $to,
    $amount,
    string $gasPrice,
    ...
    $struct = new Struct();
    $struct->add($from, new Address(self::ZPT_CONTRACT), $to, $num);
    $list = [$struct];
    ...
    $txBuilder = new TransactionBuilder();
    /** @var Transfer $tx */
    $tx = $txBuilder->makeNativeContractTx('transferFrom', $params, new Address(self::GALA_CONTRACT), $gasPrice, $gasLimit, $payer);
    $tx->tokenType = 'GALA';
    $tx->from = $from;
    $tx->to = $to;
    $tx->amount = $amount;
